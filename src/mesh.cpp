...
void Mesh::addSolution(Eigen::Ref<Eigen::MatrixXd> solutionXd)
{
  ...
  for (auto i = 0; i < m_mesh->GetNumberOfPoints(); i++) {
    solutionE->SetTuple3(i, 
        solutionXd(2 * i + 0,0), solutionXd(2 * i + 1,0), 0);
  }
  m_mesh->GetPointData()->AddArray(solutionE);
}

void Mesh::addSolution( vtkUnstructuredGrid* ug,
    Eigen::Ref<Eigen::MatrixXd> solutionXd)
{

  int nP = (int) ug->GetNumberOfPoints();

  if (solutionXd.rows() != nP * 2)
    std::runtime_error("dimension problem");

  std::cout << "solutionXd: " << solutionXd.rows() << '\n';
  std::cout << "solutionXd: " << solutionXd.cols() << '\n';
...
void Mesh::DomainDecomposition()
{
    ...
    //std::cout << "n parts = " << _nparts << std::endl;
